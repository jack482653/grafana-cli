...
**Trade-off accepted**: `GET /api/datasources` requires **Admin role only** — Viewer and Editor credentials will both see a permission error, since the response can include datasource credentials. (Correction: this was originally documented and implemented as "Editor or Admin"; a second `/code-review` pass questioned it and empirical testing against a live Grafana 7.5 instance with an Editor-role API key confirmed Editor is denied too — fixed in code, tests, and this doc.) This is called out explicitly in the spec (FR-005, Edge Cases) rather than silently avoided.
